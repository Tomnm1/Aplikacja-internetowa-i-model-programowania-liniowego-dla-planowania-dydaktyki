...
import { FieldOfStudy, FieldOfStudiesState, BackendFieldOfStudies } from '../../utils/Interfaces';
...
export const fetchFOS = createAsyncThunk<FieldOfStudy[]>('fos/fetchFOS', async () => {
    ...
    const adjustedData: FieldOfStudy[] = data.map((fos) => ({
        id: fos.fieldOfStudyId,
        name: fos.name,
    }));
    return adjustedData;
});

export const addFOS = createAsyncThunk<{ tempId: GridRowId; fos: FieldOfStudy }, FieldOfStudy>(
    'fos/addFOS',
    async (fos: FieldOfStudy) => {
        const fosData = {
            name: fos.name,
        };
        const response = await fetch(API_ENDPOINTS.FIELD_OF_STUDIES, {
...
export const updateFOS = createAsyncThunk<FieldOfStudy, FieldOfStudy>(
    'fos/updateFOS',
    async (fos: FieldOfStudy) => {
        const fosData = {
            name: fos.name,
...
export const deleteFOS = createAsyncThunk<GridRowId, GridRowId>(
    'fos/deleteFOS',
    async (id: GridRowId) => {
