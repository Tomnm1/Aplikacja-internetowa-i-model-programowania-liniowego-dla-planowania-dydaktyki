...
export const fetchBuildings = createAsyncThunk<Building[]>('buildings/fetchBuildings', async () => {
    ...
    const adjustedData: Building[] = data.map((building) => ({
        id: building.buildingId, code: building.code,
    }));
    return adjustedData;
});
...
export const addBuilding = createAsyncThunk<{
    tempId: GridRowId;
    building: Building
}, Building>('buildings/addBuilding', async (building: Building) => {
    const buildingData = {
        code: building.code,
    };
    const response = await fetch(API_ENDPOINTS.BUILDINGS, {
    ...
    const adjustedBuilding: Building = {
        id: data.buildingId, code: data.code,
    };
    return {tempId: building.id, building: adjustedBuilding};
});

export const updateBuilding = createAsyncThunk<Building, Building>('buildings/updateBuilding', async (building: Building) => {
    const buildingData = {
        code: building.code,
    };
...
const buildingSlice = createSlice({
    name: 'buildings', initialState, reducers: {
...
export const {
    setRowModesModel, setSelectedRow, clearSelectedRow, addNewBuilding, removeNewBuilding,
